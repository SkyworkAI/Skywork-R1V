...
# -*- coding: utf-8 -*-
"""
批量测试脚本 - 读取jsonl文件并调用API获取结果（非stream模式）

用法:
    python3 batch_deepresearch_nonstream.py [input_jsonl] [output_jsonl]
"""
...
def call_api(image_path, question):
    ...
    try:
        ...
        if "choices" in response_data and len(response_data["choices"]) > 0:
            ...
            print(f"获取到响应: {full_response[:100]}...")
            return {"full_response": full_response, "raw_response": response_data}
        else:
            return {"error": "响应格式不正确", "raw_response": response_data}
...
def main():
    ...
    if len(sys.argv) > 2:
        ...
        output_jsonl = "result_nonstream.jsonl"
